...
UInt32 SMCRSCard::readReg32(UInt32 reg)
{
    IOLockLock(this->regLock);
    UInt32 ret;
    if ((reg * sizeof(*this->rmmioPtr)) < this->rmmio->getLength()) { ret = this->rmmioPtr[reg]; }
    else {
...
{
    IOLockLock(this->regLock);
    if ((reg * sizeof(*this->rmmioPtr)) < this->rmmio->getLength()) { this->rmmioPtr[reg] = val; }
    else {
        this->rmmioPtr[mmPCIE_INDEX] = reg;
...
bool SMCRSCard::initialise(IOPCIDevice* device)
{
    ...
    switch (deviceID) {
        ...
        case 0x7310 ... 0x73FF:    // Navi 1x, Navi 2x
            ...
            break;
        default: SYSLOG("RSCard", "Unsupported card 0x%04X", deviceID); return false;
    }
...
IOReturn SMCRSCard::getTemperature(UInt16* data)
{
    ...
    switch (this->attributes.getFamily()) {
        case SMCRSChipFamily::SeaIslands:
        case SMCRSChipFamily::SouthernIslands:
        case SMCRSChipFamily::VolcanicIslands: return this->smu7GetTemp(data);
        case SMCRSChipFamily::ArcticIslands:
            if (this->attributes.isTHM11()) { return this->thm11GetTemp(data); }
            else {
                return this->thm9GetTemp(data);
            }
        case SMCRSChipFamily::Raven:
        case SMCRSChipFamily::Navi : return this->thm10GetTemp(data);
...
IOReturn SMCRSCard::getPower(float* data)
{
    ...
    switch (this->attributes.getFamily()) {
        case SMCRSChipFamily::SeaIslands     : return this->smu7GetPowerPMStatus(data);
        case SMCRSChipFamily::SouthernIslands:
        case SMCRSChipFamily::VolcanicIslands: return this->smu7GetPowerSMC(data);
        case SMCRSChipFamily::ArcticIslands  : return this->smu9GetPower(data);
        default                              : return kIOReturnUnsupported;
    }
}
